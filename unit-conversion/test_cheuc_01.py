"""
Unittest test suite for che_unit_converter validation.
...
The units tested are selected for typical chemical engineering applications — e.g.,
...
Steps:
    1. Add this file to the folder containing che_unit_converter.py and __init__.py.
    2. Define your acceptable tolerance.
    3. Add your test cases to the 'test_cases' and 'non_consistent_test_cases' dictionaries.
    4. Run the tests by typing "python -m unittest -v" in your IDE terminal.
"""
...
class TestCheucConversions(unittest.TestCase):
    def test_valid_conversions(self):
        for case in test_cases:
            with self.subTest(f"{case['value']} {case['from_unit']} → {case['to_unit']}"):
    ...
    def test_inconsistent_dimensions(self):
        for case in non_consistent_test_cases:
            with self.subTest(f"{case['from_unit']} → {case['to_unit']}"):
                with self.assertRaises(ValueError):
                    cheuc(case["value"], case["from_unit"], case["to_unit"])
